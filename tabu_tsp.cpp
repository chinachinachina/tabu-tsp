...
void run()
{
	...
	while (std::cin >> file_name)
	{
		...
		if (!ifs)
		{
			std::cout << "[ERROR] the file " << file_name << " does not exist" << std::endl
					  << std::endl;
		}
		else
		{
			...
			while (getline(ifs, s_temp))
			{
				...
				else if ((loc_temp = s_temp.find("SECTION")) != std::string::npos)
				{
					...
					if (dataset_type == "TSP" && dataset_edge_type == "EXPLICIT" && (dataset_edge_format == "FULL_MATRIX"))
					{
						...
						std::cout << "[INFO] deviation rate: " << (curr_solution - opt_map[dataset_name]) * 100 / opt_map[dataset_name] << "%" << std::endl;
